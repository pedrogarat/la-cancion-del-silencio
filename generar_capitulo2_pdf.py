...
class BookCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        self.drawString(54, 32, "Capítulo 2: Los Tres Hilos del Ilusionista (Dr. Em Pleh)")
...
def create_quote_box(paragraphs_text, styles, width=A4[0] - 108):
    ...
    for line in paragraphs_text:
        # Reemplazar markdown básico por etiquetas HTML de reportlab
...
def generate_chapter2_pdf(md_path, output_filename):
    ...
    # Encabezado editorial del Capítulo 2
    story.append(Paragraph("LA GUERRA DE LA HERENCIA • LIBRO I", super_title_style))
    story.append(Paragraph("Capítulo 2: Los Tres Hilos del Ilusionista", title_style))
    story.append(Paragraph("CERN, MIT y ESA: El arte de ganar la devoción de la ciencia humana", subtitle_style))
    ...
    while i < len(lines):
        ...
        if line.startswith("### 📍"):
            ...
            # Mirar si las siguientes líneas son metadatos en cursiva (*...*)
            meta_lines = []
            j = i + 1
            while j < len(lines):
...
if __name__ == "__main__":
    ...
    md_file = os.path.join(base_dir, "novela", "capitulos", "capitulo_2.md")
    out_file1 = os.path.join(base_dir, "novela", "Capitulo_2_Los_Tres_Hilos_del_Ilusionista.pdf")
    out_file2 = os.path.join(base_dir, "Capitulo_2_Los_Tres_Hilos_del_Ilusionista.pdf")
    
    generate_chapter2_pdf(md_file, out_file1)
    # Copiar a la raíz para fácil visualización
    import shutil
